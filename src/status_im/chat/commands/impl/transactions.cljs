...
(defn- render-asset [arg-index bot-db-key]
  (fn [{:keys [name symbol amount decimals] :as asset}]
    [react/touchable-highlight {:on-press #(re-frame/dispatch
     ...
     [react/view transaction-styles/asset-container
      [react/view transaction-styles/asset-main
       [react/image {:source (-> asset :icon :source)
                     :style  transaction-styles/asset-icon}]
       [react/text {:style transaction-styles/asset-symbol} symbol]
       [react/text {:style transaction-styles/asset-name} name]]
      ...
      #_[react/text {:style transaction-styles/asset-balance}
         (str (money/internal->formatted amount symbol decimals))]]]))

(def assets-separator [react/view transaction-styles/asset-separator])

(defview choose-asset-view [on-selected]
